...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        dst_train = MHISTDataset(csv_file='annotations.csv', root_dir='images', partition='train', transform=transform)
        dst_test = MHISTDataset(csv_file='annotations.csv', root_dir='images', partition='test', transform=transform)
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if isinstance(output, tuple):  # Check if output is a tuple
            output = output[0]  # Select the first element of the tuple
